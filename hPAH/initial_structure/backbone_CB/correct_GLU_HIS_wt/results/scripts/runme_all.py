...
run=int(0)
#while os.path.exists("checkpnt_%d.chk"%run):
#      run += 1
...
if os.path.isfile("checkpnt_mini_%d.chk"%(int(run)-1)):
      print ("continue from mini %d"%(int(run)-1))
      with open('checkpnt_mini_%d.chk'%(int(run)-1), 'rb') as f:
           simulation.context.loadCheckpoint(f.read())
else:
   print ("continue from all %d"%(int(run)-1))
   with open('checkpnt_all_%d.chk'%(int(run)-1), 'rb') as f:
        simulation.context.loadCheckpoint(f.read())
...
simulation.reporters.append(CheckpointReporter('checkpnt_all_%d.chk'%run, 5000))
